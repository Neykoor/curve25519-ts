...
function gf(init?: number[] | Float64Array): GF {
  var i: number, r = new Float64Array(16);
...
function add(p: Point, q: Point): void {
  var a = gf(),
    b = gf(),
    c = gf(),
    d = gf(),
...
function pack(r: Uint8Array, p: Point): void {
  var tx = gf(),
    ty = gf(),
    zi = gf();
  inv25519(zi, p[2]);
...
function scalarmult(p: Point, q: Point, s: Uint8Array): void {
...
function scalarbase(p: Point, s: Uint8Array): void {
  var q: Point = [gf(), gf(), gf(), gf()];
  set25519(q[0], X);
  set25519(q[1], Y);
  set25519(q[2], gf1);
  M(q[3], X, Y);
...
function modL(r: Uint8Array, x: Float64Array): void {
  var carry: number, i: number, j: number, k: number;
  for (i = 63; i >= 32; --i) {
    carry = 0;
...
function crypto_sign_direct(sm: Uint8Array, m: Uint8Array, n: number, sk: Uint8Array): number {
  var h = new Uint8Array(64),
    r = new Uint8Array(64);
  var i: number,
    j: number,
    x = new Float64Array(64);
  var p: Point = [gf(), gf(), gf(), gf()];
...
function crypto_sign_direct_rnd(sm: Uint8Array, m: Uint8Array, n: number, sk: Uint8Array, rnd: Uint8Array): number {
  var h = new Uint8Array(64),
    r = new Uint8Array(64);
  var i: number,
    j: number,
...
function curve25519_sign(sm: Uint8Array, m: Uint8Array, n: number, sk: Uint8Array, opt_rnd?: Uint8Array): number {
  ...
  for (var j = 0; j < 64; j++) edsk[j] = 0;

  return smlen;
}

function unpackneg(r: Point, p: Uint8Array): number {
  var t = gf(),
    chk = gf(),
    num = gf(),
    den = gf(),
...
function crypto_sign_open(m: Uint8Array, sm: Uint8Array, n: number, pk: Uint8Array): number {
...
function curve25519_sign_open(m: Uint8Array, sm: Uint8Array, n: number, pk: Uint8Array): number {
...
function checkArrayTypes(...args: Uint8Array[]): void {
  var t: string, i: number;
  for (i = 0; i < args.length; i++) {
    if ((t = Object.prototype.toString.call(args[i])) !== '[object Uint8Array]')
...
export function sharedKey(secretKey: Uint8Array, publicKey: Uint8Array): Uint8Array {
  ...
  var check = 0;
  for (var i = 0; i < 32; i++) check |= sharedKey[i];
  if (check === 0) throw new Error('invalid public key (low-order point)');
  return sharedKey;
}

export function signMessage(secretKey: Uint8Array, msg: Uint8Array, opt_random?: Uint8Array): Uint8Array {
...
export function openMessage(publicKey: Uint8Array, signedMsg: Uint8Array): Uint8Array | null {
  checkArrayTypes(signedMsg, publicKey);
  if (publicKey.length !== 32) throw new Error('wrong public key length');
  var sm = new Uint8Array(signedMsg);
  var tmp = new Uint8Array(sm.length);
  var mlen = curve25519_sign_open(tmp, sm, sm.length, publicKey);
...
export function sign(secretKey: Uint8Array, msg: Uint8Array, opt_random?: Uint8Array): Uint8Array {
